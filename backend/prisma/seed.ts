...
const expenseSets: Record<string, [string, ExpenseCategory, number][]> = {
  ...
  "Montana B13 01": [["Garantidora", "OUTROS", 1450], ["Pia", "MATERIAL", 900], ["Mao de obra", "MAO_DE_OBRA", 6200], ["Material", "MATERIAL", 5100], ["Condominio", "CONDOMINIO", 2200], ["Laudo", "LAUDO", 650], ["IPTU", "IPTU", 700], ["Comissao imobiliaria", "COMISSAO_IMOBILIARIA", 5600]],
...
async function main() {
  ...
  const properties = [
    { name: "Casa Esplanada V", purchasePrice: 75305.50, currentAppraisal: 170000, finalSalePrice: 145279.04, status: "VENDIDO", purchaseDate: "2025-01-15", saleDate: "2026-02-15", links: [["Pietro", 37652.75], ["Victor", 37652.75]] },
    { name: "Aracati 108 A", purchasePrice: 67068.27, currentAppraisal: 135000, finalSalePrice: 133000, status: "VENDIDO", purchaseDate: "2026-01-15", saleDate: "2026-09-15", links: [["Pietro", 67068.27]] },
    { name: "Jaspe 07", unit: "07", purchasePrice: 62624.60, currentAppraisal: 142000, finalSalePrice: 131000, status: "VENDIDO", purchaseDate: "2024-10-01", saleDate: "2025-07-01", links: [["Pietro", 42626.60], ["Victor", 20000]] },
    ...
    { name: "Montana B13 01", unit: "B13 01", district: "Ypiranga", purchasePrice: 62819.40, currentAppraisal: 153000, finalSalePrice: 142000, status: "VENDIDO", purchaseDate: "2026-01-15", links: [["Pietro", 31409.70], ["Jeferson", 31409.70]] },
    { name: "Recanto Jovens 02 A8", purchasePrice: 59444.27, currentAppraisal: 145000, finalSalePrice: 140000, status: "VENDIDO", purchaseDate: "2026-01-15", links: [["Bia", 59444.27]] },
  ...
  for (const item of properties) {
    ...
    const propertyData = { name: item.name, unit: "unit" in item ? item.unit : undefined, district: "district" in item ? item.district : undefined, type: PropertyType.APARTAMENTO, status: item.status as PropertyStatus, purchasePrice: item.purchasePrice, currentAppraisal: item.currentAppraisal, finalSalePrice: "finalSalePrice" in item ? item.finalSalePrice : undefined, city: "Sete Lagoas", purchaseDate: new Date(item.purchaseDate), saleDate: "saleDate" in item ? new Date(item.saleDate) : undefined, condominiumMonths: 3, condominiumMonthlyValue: 450, condominiumOldDebt: 0, condominiumTotal: 1350, registryNumber: `MAT-${item.name.slice(0, 3).toUpperCase()}`, iptuNumber: `IPTU-${item.name.length}`, notes: "Registro criado pelo seed do MVP." };
    if (existing) {
  ...
  if (!setting) await prisma.setting.create({ data: { cdiAnnualRate: 10.65, notes: "Configuracao inicial." } });
}
